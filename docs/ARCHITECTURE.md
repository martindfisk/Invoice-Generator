...
Condensed from the approved plan (`docs/adr/` records the individual decisions in detail). This
file is the terse map loaded into every agent session; the full explanations — invoice model,
mappings, Mapper mechanics, validation, send lifecycle, infrastructure, limitations, screen tour —
live in the **[handbook](handbook/README.md)**, with a generated per-field
[mapping reference](handbook/04-mapping-reference.md) (`make handbook` / `make handbook-check`).
