...
5. **Diff content remains a placeholder.** The DiffView changes are a plausible guess, not a recorded artifact — this is the demo's climax, so it must be regenerated from a real `?compliance-artifact` response once TEST credentials exist (blocked on M1 spikes a/d).
